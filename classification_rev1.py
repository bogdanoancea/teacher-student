"""
Reviewer-response experiments for the CLASSIFICATION knowledge-distillation study.
================================================================================
Companion to the submitted `classification.py`. Imports its building blocks so
the original stays untouched. Place this file next to classification.py and run:
...
# ============================================================
def exp_C8():
    print("\n" + "=" * 80); print(f"C8  MODERN TABULAR MODELS (guarded) (seeds={min(N_SEEDS,5)})"); print("=" * 80)
    n = min(N_SEEDS, 5)
    out = {'available': {}, 'results': {}}

    # ---- TabPFN (subsample to its training-size limit) ----
    try:
        ...
        for s in range(n):
            ...
            comp.append(accuracy_score(yte, clf.predict(Xte)))
            soft_full = clf.predict_proba(Xtr)
        ...
        print(f"  TabPFN: standalone acc={np.mean(comp):.4f}  as-teacher->MLP(32x2)={np.mean(as_teacher):.4f}")
    except Exception as e:
        out['available']['tabpfn'] = False
        print(f"  [skip] TabPFN not available: {e}")

    # ---- pytorch-tabular: FT-Transformer / TabNet (optional) ----
    try:
        import pandas as pd
            ...
            try:
                ...
                for s in range(n):
                    ...
                    tm = TabularModel(
                        ...
                        model_config=mkcfg(), optimizer_config=OptimizerConfig(),
                        trainer_config=TrainerConfig(max_epochs=30, accelerator='cpu',
                                                     progress_bar='none'))
                    tm.fit(train=tr)
                    pred = tm.predict(te)
                    accs.append(accuracy_score(data['y_te'], pred['prediction'].values))
                out['available'][label] = True
                out['results'][label] = {'standalone_acc': float(np.mean(accs))}
                print(f"  {label}: standalone acc={np.mean(accs):.4f}")
            except Exception as e:
                out['available'][label] = False
                print(f"  [skip] {label} run failed: {e}")
    except Exception as e:
        out['available']['pytorch_tabular'] = False
        print(f"  [skip] pytorch-tabular (FT-Transformer/TabNet) not available: {e}")

    print("  (SAINT is skipped by default to avoid a hard dependency; add a hook if installed.)")
    DUMP['C8'] = out
    return out
...
def exp_C10():
    ...
    for dsname, did in catalog.items():
        ...
        try:
            ...
            y_raw = ds.target.astype('category').cat.codes.to_numpy()
            ok = ~np.isnan(X).any(axis=1)
            X, y = X[ok], y_raw[ok]
            X = StandardScaler().fit_transform(X)
        except Exception as e:
            print(f"  [skip] could not fetch ({e})"); out[dsname] = {'skipped': str(e)}; continue
        all_data = []
        for s in range(n):
            seed = 42 + s
...
if __name__ == '__main__':
    ...
    with open('results_classification_revision_full.json', 'w') as f:
        json.dump(_native(DUMP), f, indent=2)
    print("\nWrote results_classification_revision_full.json")
