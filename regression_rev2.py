...
# R9: ADDITIONAL REAL DATASETS (OpenML, network-guarded)   (R1.14)
# ============================================================
def _load_openml_regression(spec):
    """Fetch an OpenML regression dataset robustly and return clean 2-D X, 1-D y.
    `spec` is (name, version) or (name, None). Raises on anything unsuitable so
    the caller can skip it cleanly."""
    import pandas as pd
    from sklearn.datasets import fetch_openml
    name, version = spec
    kw = dict(name=name, as_frame=True)
    if version is not None:
        kw['version'] = version
    ds = fetch_openml(**kw)
    Xdf = pd.get_dummies(ds.data, drop_first=True)          # one-hot any categoricals
    X = np.asarray(Xdf, dtype=float)
    y = pd.to_numeric(pd.Series(np.asarray(ds.target).ravel()), errors='coerce').to_numpy(float)
    ...
    if len(np.unique(y)) < 20:                              # guard against non-regression targets
        raise ValueError(f"target looks categorical (n_unique={len(np.unique(y))})")
...
def exp_R9():
    ...
    print("=" * 80)
    # (name, version) — classic numeric regression benchmarks; whichever resolve, resolve.
    catalog = [('cpu_activity', None), ('kin8nm', None),
               ('Concrete_Data', None), ('abalone', None)]
    n = min(N_SEEDS, 10)
    out = {}
    for spec in catalog:
        dsname = spec[0]
        ...
        try:                                                # ENTIRE dataset guarded: fetch + all seeds
            X, y = _load_openml_regression(spec)
            print(f"  loaded X={X.shape}, y n={len(y)}")
            all_data = []
            for s in range(n):
                seed = 42 + s
            ...
            print(f"  [skip] {dsname}: {e}")
            out[dsname] = {'skipped': str(e)}
    DUMP['R9'] = {k: ({**v['summary'], 'teacher_mse': v['teacher_mse']}
...
if __name__ == '__main__':
    ...
    out_path = 'results_regression_revision_full.json'
    merged = {}
